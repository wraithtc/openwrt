...
struct simpleDeviceEntry* findSearchedDeviceEntryByAddr(char *ieee_addr, int nw_addr)
{
    ...
    while(tmp !=NULL)
    {
        ...
        tmp=(struct simpleDeviceEntry *)tmp->root.next;
    }
    return NULL;
}
...
int RefreshSearchedDeviceList()
{
    ...
    {
        tmp2=(struct simpleDeviceEntry *)tmp->root.next;
...
struct simpleDeviceEntry* findUserEntryByUserId(char *userid)
{
    ...
    while(tmp !=NULL)
    {
        ...
        if(strcmp(tmp->userid, userid) == 0 )
        {
            ...
            return (struct simpleDeviceEntry *)tmp;
        }
        tmp=(struct UserEntry*)tmp->root.next;
    }
    return NULL;
}
...
int AddToUserEntryList(struct UserEntry *input)
{
    ...
    struct UserEntry*tmp_result= (struct UserEntry*)findUserEntryByUserId(input->userid);
    if(tmp_result == NULL)
    {
...
int RemoveFromUserEntryList(char* input_userid)
{
    ...
    output_result = (struct UserEntry*)findUserEntryByUserId(input_userid);
    if(output_result== NULL)
    {
