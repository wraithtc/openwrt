...
static inline int hash(int head, const int add)
{
    ...
    if (head >= LZW_HASH_SIZE)
        head -= LZW_HASH_SIZE;
    utilAst_assert(head >= 0 && head < LZW_HASH_SIZE);
    return head;
}
...
void put_bits(PutBitContext *s, int n, unsigned int value)
{
    ...
    utilAst_assert(n == 32 || value < (1U << n));
...
VOS_RET_E utilLzw_initEncoder(LZWEncoderState **p, UINT8 *outbuf, UINT32 outsize)
{
   LZWEncoderState *s;

    *p = (LZWEncoderState *) VOS_MALLOC_FLAGS(sizeof(LZWEncoderState), ALLOC_ZEROIZE);
    if (*p == NULL)
    {
       vosLog_error("could not allocate %d bytes for encoder state", sizeof(LZWEncoderState));
       return VOS_RET_RESOURCE_EXCEEDED;
    }
    else
    {
       vosLog_debug("%d bytes allocated for encoder state", sizeof(LZWEncoderState));
    }
    ...
    s->bufsize = outsize;
    utilAst_assert(9 <= s->maxbits && s->maxbits <= s->maxbits);
...
int utilLzw_encode(LZWEncoderState *s, const UINT8 *inbuf, UINT32 insize)
{
    UINT32 i;
    ...
    if(insize * 3 > (UINT32) (s->bufsize - s->output_bytes) * 2 + round_error){
        vosLog_error("size check failed, insize=%d (*3) outsize=%d (*2)", insize, (s->bufsize - s->output_bytes));
        return -1;
    }
...
SINT32 utilLzw_flushEncoder(LZWEncoderState * s)
{
    if (s->last_code != -1)
        writeCode(s, s->last_code);
...
void utilLzw_cleanupEncoder(LZWEncoderState **s)
{
   VOS_FREE(*s);
   *s = NULL;
}
