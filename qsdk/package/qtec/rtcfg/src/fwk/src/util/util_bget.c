...
extern char *snprintf();               /* Sun includes don't define sprintf */
#endif
  ...
  void *buf;
{
	...
	for (i = 0; i < l; i++) {
            V snprintf(bhex + i * 3, sizeof(bhex) - i * 3 - 1, "%02X ", bdump[i]);
...
static void *bexpand(bufsize size)
{
    ...
    if (cural < PoolSize) {
	np = (void *) VOS_MALLOC((unsigned) size);
    }
#ifdef EXPTRACE
...
int main(void)
{
    ...
    g_bg->bp = VOS_MALLOC(ExpIncr);
    assert(g_bg->bp != NULL);
    bpool((void *) g_bg->bp, (bufsize) ExpIncr);
#else
    g_bg->bp = VOS_MALLOC(PoolSize);
    assert(g_bg->bp != NULL);
