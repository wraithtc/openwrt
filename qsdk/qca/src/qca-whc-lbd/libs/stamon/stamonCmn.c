...
static void stamonCmnRSSIObserver(stadbEntry_handle_t entry,
    ...
    // Prepare for monitoring if legacy STA RSSI has crossed configured thresholds
    if ((reason == stadb_rssiUpdateReason_crossing) &&
         !(stadbEntry_isRRMSupported(entry))) {
        dbgf(stamonState.dbgModule, DBGDEBUG,
             "%s Received RSSI Crossing - STA RSSI crossed "
             "one of the configured thresholds ", __func__);
        stamonPrepareMonitoring(entry);
    }
}
