...
VOS_RET_E utilLzw_initDecoder(LZWDecoderState **p, UINT8 *inbuf, UINT32 inbuf_size)
{
    LZWDecoderState *s;
    int mode = FF_LZW_TIFF;  /* always use TIFF mode */
    int csize = 8;  /* the encoder side has this hardcoded, so hardcode here too */

    *p = (LZWDecoderState *) VOS_MALLOC_FLAGS(sizeof(LZWDecoderState), ALLOC_ZEROIZE);
    if (*p == NULL)
    {
       vosLog_error("could not allocate %d bytes for decoder state", sizeof(LZWDecoderState));
       return VOS_RET_RESOURCE_EXCEEDED;
    }
    else
    {
       vosLog_debug("%d bytes allocated for decoder state", sizeof(LZWDecoderState));
    }
...
SINT32 utilLzw_decode(LZWDecoderState *s, UINT8 *outbuf, UINT32 outlen)
{
    UINT32 l;
    int c, code, oc, fc;
    uint8_t *sp;
    ...
    for (;;) {
        ...
        c = lzw_get_code(s);
        if (c == s->end_code) {
            vosLog_debug("got end code %d", c);
            break;
        } else if (c == s->clear_code) {
            vosLog_debug("got clear code %d", c);
            s->cursize = s->codesize + 1;
            s->curmask = mask[s->cursize];
            s->slot = s->newcodes;
            s->top_slot = 1 << s->cursize;
            ...
            if (code == s->slot && fc>=0) {
                *sp++ = fc;
                ...
                vosLog_error("code %d greater than slot %d", code, s->slot);
                break;
            }
  ...
  the_end:
    ...
    vosLog_debug("about to return, outlen=%d l=%d\n", outlen, l);
    return outlen - l;
}



void utilLzw_cleanupDecoder(LZWDecoderState **s)
{
   VOS_FREE(*s);
   *s = NULL;
}
