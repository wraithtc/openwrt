...
struct simpleDeviceEntry * GetDevInfoByAddr( struct simpleDeviceEntry *input_list_head, char * input_ieee_addr, int nw_addr)
{
    ...
    {
        return (struct simpleDeviceEntry *)findManagedDeviceEntryByAddr(input_ieee_addr, nw_addr);
    }
    else if (input_list_head == global_searchedDeviceEntryList_head)
    {
        return (struct simpleDeviceEntry *)findSearchedDeviceEntryByAddr(input_ieee_addr, nw_addr);
    }
    else
    {
...
struct simpleDeviceEntry * GetDevInfoById(struct simpleDeviceEntry *input_list_head, char *input_deviceid)
{
    ...
    if(input_list_head == global_ManagedDeviceEntryList_head )
    {
        return (struct simpleDeviceEntry *)findManagedDeviceEntryByDeviceId(input_deviceid);
    }
    else if(input_list_head == global_searchedDeviceEntryList_head )
    {
        return (struct simpleDeviceEntry *)findSearchedDeviceEntryByDeviceId(input_deviceid);
    }
    else
    {
        printf("===[%s] the input is wrong ====\n", __func__);
...
{
    DEBUG_PRINTF("===[%s] ====\n", __func__);
    struct simpleDeviceEntry *newentry = (struct simpleDeviceEntry *)malloc_simpleDeviceEntry();
...
int RenameDevName(char *input_devid, char *input_name)
{
    ...
    struct simpleDeviceEntry* result = (struct simpleDeviceEntry*)findManagedDeviceEntryByDeviceId(input_devid);
    if(result == NULL)
    {
...
int AddFingerPrint(char *input_devid, char *input_userid, char *input_fingerprintid)
{
    ...
    struct FingerPrintEntry *newentry= (struct FingerPrintEntry *)malloc_fingerPrintEntry();
...
int RenameFingerPrint(char *input_devid, char *input_userid, char *input_fingerprintid, char *input_fingerprintname)
{
    ...
    struct FingerPrintEntry *result =(struct FingerPrintEntry *)findFingerPrintEntry(input_fingerprintid,input_devid);
    if(result == NULL)
    {
...
int GetFingerPrintsByUserIdDeviceId(char *input_devid, char* input_userid, struct FingerPrintEntry* buf)
{
    ...
    while(tmp!=NULL)
    {
        ...
        tmp=(struct FingerPrintEntry*)tmp->root.next;
    }
...
int savelog2local(char *input_time, int opratetype, char * input_usrid, char * devid)
{
    ...
    struct LogEntry *newentry = (struct LogEntry *)malloc_logEntry();
