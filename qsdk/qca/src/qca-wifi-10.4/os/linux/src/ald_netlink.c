...
int ald_record_nac(wlan_if_t vap, u_int8_t *bssid, u_int8_t *macaddr, u_int8_t rssi)
{
    struct net_device *dev;
    struct ald_nac_info info;

    info.cmd = IEEE80211_ALD_NAC;
    dev = ((osif_dev *)vap->iv_ifp)->netdev;
    ...
    ald_notify(vap, IEEE80211_ALD_NAC, sizeof(info), &info);
    return 0;
}
