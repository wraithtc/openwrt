...
__repacd_wifimon_measure_rate() {
    local rate_to_CAP bestap bestap_otherband
    ...
    if [ -n "$rate_to_CAP" ]; then
        if [ "$rate_num" -lt "$rate_samples" ]; then
            __repacd_wifimon_debug "Sample #$rate_num CAP SNR = $CAP_snr Rate to CAP = $rate_to_CAP Mbps"
            ...
            local rate_to_CAP_median my_uplink_rate_median CAP_snr_median bestap_bssid_list restarted=0 config_success=0
            ...
            __repacd_wifimon_debug "Median CAP SNR = $CAP_snr_median Rate to CAP = $rate_to_CAP_median Mbps"
                ...
                if [ $CAP_snr_threshold -gt 0 -a $CAP_snr_median -ge $CAP_snr_threshold ]; then
                    ...
                    if __repacd_wifimon_is_cap_serving \
                        ...
                        if [ "$rate_to_CAP_median" -lt "$rate_min" ]; then
                            ...
                            __repacd_wifimon_get_max_duplicated_entry $bestap_filename bestap_bssids
                            __repacd_wifimon_parse_best_bssid_list "$bestap_bssid_list"
                            __repacd_wifimon_config_best_ap $measurement_sta_iface $measurement_sta_iface_config_name restarted config_success
                            if [ "$restarted" -eq 0 ]; then
