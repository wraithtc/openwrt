...
pl_base64_encode_buffer(PLBase64Encoder *data, const unsigned char *in, UINT32 size)
{
   ...
   while (in < end)
   {
      ...
      {
         utilAst_assert(data->output_length <= data->output_buflen);
...
static VOS_RET_E pl_base64_encode_flush (PLBase64Encoder *data)
{
   SINT32 i = data->in_buffer_count;

   vosLog_debug("in_buffer_count=%d", i);
...
pl_base64_create_encoder (UINT32 line_length, UINT32 output_buflen)
{
   ...
   data = VOS_MALLOC_FLAGS(sizeof(PLBase64Encoder), ALLOC_ZEROIZE);
   if (data == NULL)
   {
      return NULL;
   }
   ...
   data->output_buffer = (char *) VOS_MALLOC_FLAGS(output_buflen, ALLOC_ZEROIZE);
