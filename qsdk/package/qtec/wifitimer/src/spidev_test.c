...
static unsigned char getxor(unsigned char *pbuff,unsigned int len)
{
...
static void SpiSendByte(int fd, uint8_t value)
{
	int ret;
	uint8_t tx[] = {
...
static void SpiRecv(int fd, char* recvBuf, int len)
{
	int ret;
	uint8_t tx[] = {
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF/*, 0xFF,
	...
	for (ret = 0; ret < ARRAY_SIZE(tx); ret++) {
		if (!(ret % 6))
			puts("");
		printf("%.2X ", recvBuf[ret]);
	}
	puts("");
}

static void SpiRecvByte(int fd, char recvBuf)
{
	int ret;
	uint8_t tx[] = {
		0xFF, /*, 0xFF,
