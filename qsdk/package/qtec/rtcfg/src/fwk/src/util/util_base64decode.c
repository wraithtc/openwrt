   ...
   while (length > 0)
   {
      while (i < 4 && length > 0)
      {
	    ...
	    if (i < 4)
       {
	    ...
	    utilAst_assert((out - data->output_buffer + 3) <= (SINT32) data->output_buflen);
 ...
 */
static VOS_RET_E pl_base64_decode_flush (PLBase64Decoder *data)
{
    int count;
...
{
    return ((PLBase64Decoder *) VOS_MALLOC_FLAGS(sizeof(PLBase64Decoder), ALLOC_ZEROIZE));
}
...
VOS_RET_E utilB64_decode(const char *base64Str, UINT8 **binaryBuf, UINT32 *binaryBufLen)
{
    ...
    {
        vosLog_error("could not create decoder, cannot decode");
        return VOS_RET_INTERNAL_ERROR;
    }
    ...
    data->output_buffer = VOS_MALLOC_FLAGS(data->output_buflen, ALLOC_ZEROIZE);
    
    if (data->output_buffer == NULL)
    {
       vosLog_error("could not allocate output buffer of %d bytes", data->output_buflen);
