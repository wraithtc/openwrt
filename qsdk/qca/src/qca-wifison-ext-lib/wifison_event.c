...
struct soneventAppState {
    int socket;
    bool eventrigerflag[SON_MAC_EVENT];
}soneventAppS;
...
int wifison_event_get(struct sonEventInfo *event)
{
    ...
    switch (message->cmd) {
        case CLIENT_START:
            wifison_server_start();
            for (i = 0; i < SON_MAC_EVENT; i++)
            {
                if (soneventAppS.eventrigerflag[i]) //If cline restart after daemon start, sent the evnt to re-register where already registered.
                {
                    wifison_event_register(i);
                    err = EVENT_OK;
                    event->eventMsg = CLIENT_START;
                }
            }
            break;
